"""
Lambda function for document deletion
Deletes documents and metadata for authorized users
"""
...
class DeleteError(Exception):
    """Custom exception for delete errors"""
    pass
...
def get_document_metadata(document_id: str) -> Dict[str, Any]:
    ...
    Raises:
        DeleteError: If document not found or DynamoDB operation fails
    """
    try:
        ...
        if 'Item' not in response:
            raise DeleteError(f'Document not found: {document_id}')
        ...
        raise DeleteError(error_msg)


@xray_recorder.capture('authorize_delete')
@retry_avp_operation
def authorize_delete(
    ...
    """
    Checks delete permission via Amazon Verified Permissions
    ...
    try:
        ...
        if logger:
            logger.error(
                ...
                f"Authorization error for delete: {str(e)}",
                error=str(e),
                document_id=document_id
            )
        else:
...
@xray_recorder.capture('delete_from_s3')
@retry_s3_operation
def delete_from_s3(s3_key: str) -> None:
    ...
    Raises:
        DeleteError: If S3 delete operation fails
    """
    try:
        ...
        error_msg = f"S3 delete operation failed: {str(e)}"
        print(json.dumps({
...
def delete_metadata(document_id: str) -> None:
    ...
    Raises:
        DeleteError: If DynamoDB delete operation fails
    """
    try:
        ...
        print(json.dumps({
            'event_type': 'metadata_delete_success',
            'document_id': document_id
        }))
        
    except ClientError as e:
        error_msg = f"DynamoDB delete operation failed: {str(e)}"
        print(json.dumps({
            'event_type': 'metadata_delete_error',
            'error': error_msg,
            'document_id': document_id
        }))
        raise DeleteError(error_msg)
...
def log_operation(
    ...
    Args:
        user_id: User identifier
        action: Action performed (delete, etc.)
        document_id: Document identifier
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    Validates authorization and deletes document from S3 and metadata from DynamoDB.
    ...
    Returns:
        API Gateway response with deletion confirmation
    """
    # Create structured logger
    logger = create_logger('document-delete', context)
    ...
    logger.info(EventType.LAMBDA_INVOCATION, "Delete Lambda invoked")
    ...
    try:
        ...
        if not user_id:
            raise DeleteError('Missing user identity from authorizer')
        ...
        if not document_id:
            raise DeleteError('Missing required path parameter: document_id')
        ...
        if not s3_key:
            raise DeleteError('Invalid document metadata: missing s3_key')
        ...
        logger.error(
            ...
            f"Unexpected error in delete handler: {str(e)}",
            error=str(e)
        )
